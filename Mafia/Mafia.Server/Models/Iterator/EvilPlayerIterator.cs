namespace Mafia.Server.Models.Iterator
{
    ...
    public class EvilPlayerIterator : IPlayerIterator // Concrete Iterator
    {
        private readonly List<Player> _entries;
        private int _currentIndex = -1;

        public EvilPlayerIterator(IEnumerable<Player> entries)
        {
            // Filter players by alignment "Evil"
            _entries = entries.Where(player => player.Role.Alignment == "Evil").ToList();
